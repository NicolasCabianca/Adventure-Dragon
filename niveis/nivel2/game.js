class Game {
  constructor() {
    ...
    this.EspF = createSprite(1,110,10,10)
    this.EspF.addImage("F",EspFImg)
    this.EspF.scale = 3
    this.EspF.visible = false;

    this.adss = createSprite(1,110,10,10)
    this.adss.addImage("2",AsDImg)
    this.adss.scale = 3
    this.adss.visible = false;

    this.btt = createSprite(1,110,10,10)
    this.btt.addImage("2",BImg)
    this.btt.scale = 3
    this.btt.visible = false;

    this.mgg = createSprite(1,110,10,10)
    this.mgg.addImage("2",MgImg)
    this.mgg.scale = 3
    this.mgg.visible = false;
    ...
    this.posbau = [
      ...
      { x:width*0.5 , y: height * 3 },
      { x:width*1 , y: height * 3 },
      { x:width*0.9 , y: height * 1 },
  ...
  play(lista) {
    if (lista.length > 0) {
      ...
      if (
        ...
        console.log("antes de trocar:",bau.position.y)

        bau.position.x = this.posbau[iB % this.posbau.length].x;
        bau.position.y = this.posbau[iB % this.posbau.length].y;
        console.log("depois de trocar:",bau.position.x)
        console.log("depois de trocar:",bau.position.y)
        iB++;
      }
  ...
  mostrarBau() {
    bau = createSprite(width / 2, height / 1, 100, 100);
    bau.addImage(bauImg);
    bau.scale = 3.5;
  ...
  Controle() {
    ...
    if (player.invAberto) {
      ...
      if(player.EspF){
        this.EspF.position.x = player.posX - 250
        this.EspF.position.y = player.posY - 50
        this.EspF.visible = true
        this.EspF.scale = 2.2
        this.EspF.depth = 9999999
      }
      if(player.ads){
        this.adss.position.x = player.posX - 250
        this.adss.position.y = player.posY - 50
        this.adss.visible = true
        this.adss.scale = 2.2
        this.adss.depth = 9999999
      }
      if(player.bt){
        this.btt.position.x = player.posX - 250
        this.btt.position.y = player.posY - 50
        this.btt.visible = true
        this.btt.scale = 2.2
        this.btt.depth = 9999999
      }
      if(player.mg){
        this.mgg.position.x = player.posX - 250
        this.mgg.position.y = player.posY - 50
        this.mgg.visible = true
        this.mgg.scale = 2.2
        this.mgg.depth = 9999999
      }
      if (player.temCPT5) {
    ...
    (
      ...
      console.log("atacando sem especial")
      console.log("espF", player.EspF)
    ...
    {
      console.log("atacando com especial")
      player.atacando = true;
      sprites[index].changeImage("ataqueE");
      player.ani = "ataqueE";
      player.update();
    }
    ...
    /*
    if(keyDown("E") && player.ads == true){
      sprites[index].changeImage("ataqueE");
      player.ani = "ataqueE";
      player.update();
    }
    if(keyDown("E") && player.bt == true){
      sprites[index].changeImage("ataqueB");
      player.ani = "ataqueE";
      player.update();
    }
    if(keyDown("E") && player.mg == true){
      sprites[index].changeImage("ataqueM");
      player.ani = "ataqueE";
      player.update();
    }*/
    if(keyWentUp("E")){
        ...
        if (!player.colisao.isTouching(cavaleirofan[n])) {
          ...
          if (sprites[i - 1].isTouching(cavaleirofan[n])) {
            if (atacando) {
              ...
              if(!player.EspF && player.personagem == 1){
                player.EspF = true;
                console.log("EspF") 
              }
              if(!player.ads && player.personagem == 2){
                player.ads = true;
                console.log("ads") 
              }
              if(!player.mg && player.personagem == 4){
                player.mg = true;
                console.log("mg") 
              }
